...
public class Main {
    ...
    private static void testAll(int[][] bordered) throws Exception {
        ...
        for (int run = 1; run <= 10; run++) {
            ...
            Sequential seq = new Sequential(N, M, n, bordered, convolutionMatrix);
            int[][] seqRes = seq.run();
            long end = System.nanoTime();
            seqTimes[run - 1] = end - start;
            writeToFile(seqRes, "sequential");
            //System.out.println("Secvential: " + (end - start) + " ns");

            // ORIZONTAL
            start = System.nanoTime();
            HorizontalAllocatedThreads hor = new HorizontalAllocatedThreads(N, M, n, p, bordered, convolutionMatrix);
            int[][] horRes = hor.run();
            end = System.nanoTime();
            horTimes[run - 1] = end - start;
            writeToFile(horRes, "horizontal");
            //System.out.println("Orizontal:  " + (end - start) + " ns");

            // VERTICAL
            start = System.nanoTime();
            VerticalAllocatedThreads ver = new VerticalAllocatedThreads(N, M, n, p, bordered, convolutionMatrix);
            int[][] verRes = ver.run();
            end = System.nanoTime();
            verTimes[run - 1] = end - start;
            writeToFile(verRes, "vertical");
            //System.out.println("Vertical:   " + (end - start) + " ns");

            // VERIFICARE
            boolean hOk = compareFiles("output_sequential.txt", "output_horizontal.txt");
            boolean vOk = compareFiles("output_sequential.txt", "output_vertical.txt");

            //System.out.println("Verificare orizontala: " + (hOk ? "CORECTA" : "GRESITA"));
            //System.out.println("Verificare verticala:  " + (vOk ? "CORECTA" : "GRESITA"));
        }

        // Calcul medii
        long seqAvg = Arrays.stream(seqTimes).sum() / 10;
        long horAvg = Arrays.stream(horTimes).sum() / 10;
        long verAvg = Arrays.stream(verTimes).sum() / 10;

        System.out.printf("N=%d M=%d n=m=%d%n", N, M, n);
        System.out.printf("Secvential: %d ns%n", seqAvg);
        System.out.printf("%d - vertical (coloane): %d ns%n", p, verAvg);
