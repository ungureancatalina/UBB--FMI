...
const EmployeePage = () => {
    ...
    const handleComplete = async () => {
        ...
        await axios.put(`http://localhost:5000/api/task/${selectedTaskId}/complete`);
    ...
    useEffect(() => {
        ...
        const handle = (msg: string) => {
            console.log(`Employee notification:`, msg);
        };

        notificationCenter.subscribe(user.id, handle);
        return () => notificationCenter.unsubscribe(user.id, handle);
    }, []);
                ...
                <div className="mt-12 w-full flex flex-col items-center gap-8">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 w-full max-w-2xl justify-center">
                        <button
                            ...
                            className={button + ' w-full md:w-1/2 bg-green-700 hover:bg-green-800'}
                        >
                            Set Task As Finished
                        </button>
                        <button
                            onClick={handleInProgress}
                            className={button + ' w-full md:w-1/2 bg-orange-700 hover:bg-orange-800'}
                        >
