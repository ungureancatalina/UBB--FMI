...
public class InscriereController implements Initializable, IObserver {
    ...
    public void handleInscriere() {
        ...
        try {
            ...
            if (echipa == null) {
                ...
                echipa = server.findEchipaByNume(echipaNume);
            }

            Participant participantNou = new Participant(nume, cnp, capacitateMoto, echipa);
            server.inscriereParticipant(participantNou);

            Participant participantCuId = server.findByNume(nume);
            ...
            for (Cursa cursa : curse) {
                if (cursa.getCapacitateMinima() <= capacitateMoto && capacitateMoto <= cursa.getCapacitateMaxima()) {
                    server.adaugaParticipantLaCursa(new ParticipantCursa(participantCuId, cursa));
                    break;
                }
            }

            showAlert("Succes", "Participantul a fost inscris și înscris la cursă!");
            updateParticipantCursaTable();
    ...
    private void updateParticipantCursaTable() {
        try {
            ...
            if (participantCursaList != null) {
                tableViewParticipantiCurse.setItems(FXCollections.observableArrayList(participantCursaList));
            } else {
                tableViewParticipantiCurse.setItems(FXCollections.observableArrayList());
            }
            columnNumeCursa.setCellValueFactory(cellData -> new SimpleStringProperty(
                    cellData.getValue().getCursa() != null ? cellData.getValue().getCursa().getNume() : "N/A"
            ));
            columnNumeParticipant.setCellValueFactory(cellData -> new SimpleStringProperty(
                    cellData.getValue().getParticipant() != null ? cellData.getValue().getParticipant().getNume() : "N/A"
            ));
        } catch (Exception e) {
    ...
    @Override
    public void adauga(ParticipantCursa participantcursa) throws Exception {
        updateParticipantCursaTable();
        showAlert("Notificare", "A fost adăugat un: " +participantcursa.getCursa().getNume()+ participantcursa.getParticipant().getNume());
    }
}