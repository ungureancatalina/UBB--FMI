    ...
    public class ClientJsonWorker : IObserver
    {
        ...
        public void Run()
        {
            ...
            while (connected)
            {
                try
                {
                    ...
                    Console.WriteLine("Am primit: " +requestJson);
        ...
        private void SendResponse(Response response)
        {
            ...
            try
            {
                ...
                Console.WriteLine("Am trimis: " + json);
                lock (stream)
                {
            ...
            {
                Console.WriteLine("Eroare la trimitere: " + e.Message);
                connected = false;
            }
        }
        ...
        private Response HandleAdaugaParticipantLaCursaRequest(Request request)
        {
            try
            {
                var participantCursa = request.participantCursa;
                if (participantCursa == null || participantCursa.Participant == null || participantCursa.Cursa == null)
                {
                    return JsonProtocolUtils.CreateErrorResponse("Participant sau Cursa null la adaugare participant la cursa.");
                }

                server.AdaugaParticipantLaCursa(participantCursa);
                return JsonProtocolUtils.CreateOkResponse();
            }
            catch (Exception e)
            {
